																							...
																							/*name may contain spaces on MAC ftpd!*/
{
	...
	*link=0;		/*No link*/
					...
					if (found) {
						...
						if (found) {
							...
							if (isadigit(monthpos[0])) {
								if (!daybeforemonth) {
									if (DecodeNumber(&monthpos,&num)) {	/*Tag �berspringen*/
	...
	if (minmonthpos!=NULL) {
		...
		if (DecodeNumber(&p,&num)) {	/*Stunde bzw. Jahr �berspringen*/
			p--;
			ch=p[0];
			p++;
			if (ch==':' || (ch=='.' && isadigit(p[0])))
				{
					...
					p2=p;
					while (p[0]==' ') p++;
					if (wcslen(p)>5 && DecodeNumber(&p,&num)) {	/*Stunde �berspringen*/
			...
			if (monthpos) {
				do {
					...
					if (monthpos!=lpstr) {
						if (wcschr(L" \t-,/",monthpos[3])==NULL) {
							found=false;
							/*Deutsch kann auch sein 'Juni' 'M�rz,' etc.*/
							/*-r-sr-xr-x	 1 lp				 bin					16384 27. Juni 1997 enable
							lr-xr-xr-t	 1 root			 sys						 20 16. M�rz, 15:20 endif -> /opt/ansic/bin/endif*/
							if (wcschr(L" \t-,/",monthpos[4])!=NULL) {
								found=true;
								incr=4;
							}
			...
			if (t.wDay>0 && lp1[0]) {
				...
				if (lp) {
					...
					if (lp) {		 /*Zeit*/
						...
						lp1=lp2+1;
						while (lp1[0]==' ') lp1++;
						if (DecodeNumber(&lp1,&num)) {	/*Stunde �berspringen*/
							t.wHour=num;
							lp1--;
							ch=lp1[0];
							lp1++;
	...
	*UnixAttr=0;
	...
	if (pstr2) {
		...
		*attr=0;
		...
		if (Permissions[0]=='l') {		// k�nnte beides sein!
